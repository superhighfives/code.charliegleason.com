---
slug: public-private-remix
title: Public private Remix
links:
...
- Install your public Remix site's GitHub repo as an NPM dependency with `npm install git://github.com/USER_NAME/REPO_NAME.git#branch` (usually main)
...
```json
{
  ...
  "scripts": {
    ...
    "sync": "concurrently \"npm run sync:files:routes\" \"npm run sync:files:assets\" \"npm run sync:files:data -- -w\"",
...
Sync your various files.

<div className="not-prose code">
  ```bash
  concurrently \"npm run sync:files:routes\" \"npm run sync:files:assets\" \"npm run sync:files:data -- -w\"
  ```
</div>
...
When you're ready to deploy your site, you'll need to set up a new Cloudflare Pages project [on the Cloudlare dashboard](https://dash.cloudflare.com/). Navigate to **Workers & Pages** > **Create Application** > **Pages** > **Connect to Git**.

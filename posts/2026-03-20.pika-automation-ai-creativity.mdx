...
So I did. I set up a mono-repo that pulls together the Pika source, a release tool I built called [releasecast](https://github.com/superhighfives/releasecast), the marketing assets, and the website. Then I wrote Claude Code slash commands - `/release`, `/screenshots`, `/setup` - that orchestrate the whole thing.
...
I've been thinking about palettes - saving and organising groups of colours, not just individual picks. CloudKit sync so your colour history follows you across machines. Better contrast workflows, maybe APCA by default. A proper Raycast extension.

Some of those are big. Some of those are small. All of them are more fun to think about now that the path from idea to release is short.

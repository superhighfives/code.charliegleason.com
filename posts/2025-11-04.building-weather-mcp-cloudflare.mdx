---
slug: building-weather-mcp-cloudflare
title: Building a weather MCP server on Cloudflare Workers
...
```bash command
npm install -D @types/node agents typescript wrangler zod
```
...
Update your `package.json` to use ES modules by adding `"type": "module"` and configuring the main entry point:
...
The `"type": "module"` field is crucial - it tells Node.js to treat `.js` files as ES modules, which is required for the MCP SDK.

Set up TypeScript by creating `tsconfig.json`:
...
We're using `"moduleResolution": "Bundler"` because Cloudflare Workers uses Wrangler with esbuild for bundling. This is the recommended configuration for bundled environments and provides better type checking than Node16 resolution. The `types` field references Cloudflare Workers types, which are automatically included with Wrangler.
...
The `compatibility_date` should be set to a recent date (at least `"2024-09-23"`) to ensure proper Node.js built-in module resolution. The `nodejs_compat` flag enables Node.js APIs in the Workers runtime. The `observability` section enables logging and metrics for debugging.

Now create a `src` directory for your code:
...
This compiles `src/index.ts` to `build/index.js`, which is what Wrangler will deploy.
